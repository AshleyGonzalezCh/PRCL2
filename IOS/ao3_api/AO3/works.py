...
class Work:
    ...
    def nchapters(self):
        ...
        # Busca el elemento con la clase "chapters"
        chapters = self._soup.find("dd", {"class": "chapters"})
        
        # Si el elemento existe, procesa el número de capítulos
        if chapters is not None:
            # Obtiene el número de capítulos de la cadena y lo convierte a entero
            return int(self.str_format(chapters.string.split("/")[0]))
        
        # Si no se encuentra el elemento, devuelve 0
        return 0
    ...
    def expected_chapters(self):
        ...
        Returns:
            int: number of chapters or None if not provided
        """
        
        # Verifica si self._soup no es None antes de intentar buscar el elemento
        ...
        if chapters is not None:
            # Extrae el número de capítulos esperados
    ...
    def words(self):
        ...
        if words is not None:
            # Limpia las comas y convierte a int
            cleaned_words = words.string.replace(",", "").strip()  # Elimina las comas y espacios
            return int(self.str_format(cleaned_words))  # Convierte la cadena limpia a int
        return 0
    ...
    def date_published(self):
        ...
        """
        # Busca el elemento <dd class="published">
        dp_elem = self._soup.find("dd", {"class": "published"})
        
        # Si el elemento no se encuentra, retorna un valor por defecto o None
        if dp_elem:
            dp = dp_elem.string
            return datetime(*list(map(int, dp.split("-"))))
        else:
            return None  # O un valor por defecto si prefieres manejarlo de otra forma
